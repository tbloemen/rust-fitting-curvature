...
pub fn project_to_2d(
    ...
    if curvature > 0.0 {
        let coords = project_spherical(points, n_points, ambient_dim, curvature, projection);
        projected.copy_from_slice(&coords);
...
/// Project spherical points to 2D using the given projection method.
/// The points are first aligned so the data centroid points toward the south
/// pole, then the requested projection is applied. Returns a flat `[x0, y0,
/// x1, y1, …]` array of length `2 * n_points`.
#[expect(
    clippy::many_single_char_names,
    reason = "r,z,s,x,y are conventional geometry projection variables (radius, z-axis, scale, coordinates)"
)]
fn project_spherical(
    ...
    match projection {
        SphericalProjection::Stereographic => {
            for i in 0..n_points {
                let offset = i * ambient_dim;
                let z = aligned[offset];
                let mut denom = r - z;
                if denom.abs() < 1e-12 {
                    denom = 1e-12;
                }
                let s = r / denom;
                let x = s * aligned[offset + 1] / r;
                let y = if ambient_dim > 2 {
                    s * aligned[offset + 2] / r
                } else {
                    0.0
                };
                projected[i * 2] = x;
