#!/bin/sh
# Launch the LOSS-EXPERIMENT 5000-sample pareto runs locally, without SLURM.
#
# Local (non-SLURM) counterpart of slurm/submit_all_5000.sh: the loss-weight
...
# / EXPERIMENTS / LOSS_GEOMETRIES (or via env) to run a subset.
#
...
#   sh run_5000_local_all.sh
#   DATASETS="tree mnist" sh run_5000_local_all.sh            # subset of datasets
#   EXPERIMENTS="all_free" sh run_5000_local_all.sh           # subset of experiments
#   THREADS=8 sh run_5000_local_all.sh                        # cap worker threads
#   RESULTS_DIR=/data/out sh run_5000_local_all.sh            # where the JSONL lands
#   LOG_DIR=/data/logs sh run_5000_local_all.sh               # where per-run logs land
...
for ds in $DATASETS; do
  for ex in $EXPERIMENTS; do
    for geo in $LOSS_GEOMETRIES; do
      run_cfg "$ex" "$ds" "$geo"
    done
  done
done

echo "==> All loss-experiment 5000-sample runs complete. Results in $RESULTS_DIR"
