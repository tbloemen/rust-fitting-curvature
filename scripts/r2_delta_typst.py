...
# Region order follows `r2::build_regions` in crates/analysis/src/r2.rs, not the
# JSONL's alphabetical key order: `all`, then the three preference families,
# then one region per objective, so the table reads in the same order as every
...
def build(table, n, datasets, settings):
    ...
    lines = [
        ...
        # Thirteen columns of numbers do not fit a page, and a `#figure` is not
        ...
        f"      align: ({', '.join(align)}),",
        "      stroke: none,",
        # Thirteen columns at the default 5pt side inset spend ~4cm of the text
