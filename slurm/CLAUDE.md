...
Everything here submits **embedding** work — that is the only thing that still needs a cluster. The `run_hv.sh` / `submit_hv.sh` pair (Monte-Carlo hypervolume) was deleted once that stage was ported to Rust: the 22-way array it used to need now runs in ~12s locally (`crates/analysis`, `hv stats`). Don't reintroduce a SLURM job for post-hoc analysis of the results JSONL without timing it locally first — the sweeps are O(n²) per iteration and genuinely need the cluster, the analysis is a Monte-Carlo integral over a few hundred front points and does not.
